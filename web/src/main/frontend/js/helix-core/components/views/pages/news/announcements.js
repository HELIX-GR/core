...
class Announcements extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.getPosts(1, 100, EnumPostCategory.Announcement);
  }
    ...
    return posts.map((p) => {
      ...
      return (
        <Link key={`post-${p.id}`} to={buildPath(DynamicRoutes.POST_PAGE, [p.id])} className="cards__item cards__item--text cards__item--events">
          ...
          <span className="cards__item__button">{_t({ id: 'buttons.card.announcement-more' })}</span>
        </Link>
      );
    });
  }
  ...
  render() {
    ...
    const { posts } = pages[EnumPostCategory.Announcement];
          ...
          <div className="page__wrapper">
            ...
            <div className="page__content">
              <ul className="page__breadcrumbs">
                ...
                <li><a href="#">{_t({ id: 'breadcrumb.announcement' })}</a></li>
              </ul>
...
export default injectIntl(ReactRedux.connect(mapStateToProps, mapDispatchToProps, mergeProps)(Announcements));
