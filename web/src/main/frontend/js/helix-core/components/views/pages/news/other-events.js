...
class OtherEvents extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.getPosts(1, 100, EnumPostCategory.OtherEvent);
  }
    ...
    return posts.map((p) => {
      ...
      return (
        <Link key={`post-${p.id}`} to={buildPath(DynamicRoutes.POST_PAGE, [p.id])} className="cards__item cards__item--text cards__item--events">
          ...
          <span className="cards__item__button">{_t({ id: 'buttons.card.other-event-more' })}</span>
        </Link>
      );
    });
  }
  ...
  render() {
    ...
    const { posts } = pages[EnumPostCategory.OtherEvent];
          ...
          <div className="page__wrapper">
            ...
            <div className="page__content">
              <ul className="page__breadcrumbs">
                ...
                <li><a href="#">{_t({ id: 'breadcrumb.other-event' })}</a></li>
              </ul>
